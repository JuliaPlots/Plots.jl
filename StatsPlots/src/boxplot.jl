...
@recipe function f(
    ...
    for (i, glabel) in enumerate(sort(glabels; by = sort_labels_by))
        # filter y
        values = y[filter(i -> PlotsBase.Commons._getvalue(x, i) == glabel, 1:length(y))]
        ...
        hw = 0.5 * PlotsBase.Commons._getvalue(bw, i) # Box width
        HW = 0.5 * PlotsBase.Commons._getvalue(ww, i) # Whisker width
