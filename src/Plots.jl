module Plots
  ...
  function load_default_backend()
      ...
      PlotsBase.backend(PlotsBase.CURRENT_BACKEND.sym)
  end
  ...
  function diagnostics(io::IO = stdout)
    ...
    if (be = PlotsBase.backend_name()) === :none
  ...
  @setup_workload begin
      ...
      for i in setdiff(1:n, PlotsBase._backend_skips[backend_name()], PlotsBase._animation_examples)
          PlotsBase._examples[i].external && continue
          (imp = PlotsBase._examples[i].imports) === nothing || push!(imports, imp)
