...
function plotly_series_segments(series::Series, plotattributes_base::KW, x, y, z, clims)
    ...
    for (k, segment) in enumerate(segments)
        ...
        if hasmarker
            ...
            try
                global mcolor_next = rgba_string(
                    plot_color(
            ...
            try
                global lcolor_next = rgba_string(
                    plot_color(
            ...
            plotattributes_out[:marker] = KW(
                ...
                :color =>
                    needs_scatter_fix ?
                    (
                        i === length(segments) ? [mcolor, mcolor_next] :
                        [mcolor, "rgba(0, 0, 0, 0.000)"]
                    ) : mcolor,
                :line => KW(
                    :color =>
                        needs_scatter_fix ?
                        (
                            i === length(segments) ? [lcolor, lcolor_next] :
                            [lcolor, "rgba(0, 0, 0, 0.000)"]
                        ) : lcolor,
