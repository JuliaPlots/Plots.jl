...
@noinline function gr_getcolorind(c)
    gr_set_transparency(float(alpha(c)))
    convert(Int, GR.inqcolorfromrgb(red(c), green(c), blue(c)))
end

@noinline gr_set_linecolor(c)   = GR.setlinecolorind(gr_getcolorind(_cycle(c,1)))
@noinline gr_set_fillcolor(c)   = GR.setfillcolorind(gr_getcolorind(_cycle(c,1)))
@noinline gr_set_markercolor(c) = GR.setmarkercolorind(gr_getcolorind(_cycle(c,1)))
@noinline gr_set_textcolor(c)   = GR.settextcolorind(gr_getcolorind(_cycle(c,1)))
@noinline gr_set_transparency(α::Real) = GR.settransparency(clamp(α, 0, 1))
@noinline gr_set_transparency(::Nothing) = GR.settransparency(1)
@noinline gr_set_transparency(c, α) = gr_set_transparency(α)
@noinline gr_set_transparency(c::Colorant, ::Nothing) = gr_set_transparency(c)
@noinline gr_set_transparency(c::Colorant) = GR.settransparency(alpha(c))
...
@noinline gr_set_arrowstyle(s::Symbol) = GR.setarrowstyle(get(_gr_arrow_map, s, 1))
...
@noinline gr_inqtext(x, y, s::Symbol) = gr_inqtext(x, y, string(s))

@noinline function gr_inqtext(x, y, s)
...
@noinline gr_text(x, y, s::Symbol) = gr_text(x, y, string(s))

@noinline function gr_text(x, y, s)
    if length(s) >= 2 && s[1] == '$' && s[end] == '$'
...
@noinline gr_x_axislims(sp::Subplot) = axis_limits(sp, :x)
@noinline gr_y_axislims(sp::Subplot) = axis_limits(sp, :y)
@noinline gr_z_axislims(sp::Subplot) = axis_limits(sp, :z)
@noinline gr_xy_axislims(sp::Subplot) = gr_x_axislims(sp)..., gr_y_axislims(sp)...

@noinline function gr_lims(sp::Subplot, axis::Axis, adjust::Bool, expand = nothing)
    if expand !== nothing
...
@noinline function gr_fill_viewport(vp::AVec{Float64}, c)
    GR.savestate()
    GR.selntran(0)
    GR.setscale(0)
...
@noinline normalize_zvals(args...) = nothing
@noinline function normalize_zvals(zv::AVec, clims::NTuple{2, <:Real})
...
# draw ONE Shape
@noinline function gr_draw_marker(xi, yi, msize, shape::Shape)
    sx, sy = coords(shape)
...
@noinline function gr_draw_marker(xi, yi, msize::Number, shape::Symbol)
...
@noinline function gr_set_line(lw, style, c) #, a)
...
@noinline function gr_set_fill(c) #, a)
...
@noinline function gr_set_font(f::Font; halign = f.halign, valign = f.valign,
...
@noinline function gr_nans_to_infs!(z)
...
@noinline function gr_viewport_from_bbox(sp::Subplot{GRBackend}, bb::BoundingBox, w, h, viewport_canvas)
    viewport = zeros(4)
...
@noinline function gr_set_viewport_cmap(sp::Subplot)
    GR.setviewport(
...
@noinline function gr_set_viewport_plotarea()
    GR.setviewport(
        viewport_plotarea[1],
        viewport_plotarea[2],
        viewport_plotarea[3],
...
@noinline function gr_set_viewport_polar()
    xmin::Float64, xmax::Float64, ymin::Float64, ymax::Float64 = viewport_plotarea
    ymax -= 0.05 * (xmax - xmin)
...
@noinline function gr_update_colorbar!(cbar::GRColorbar, series::Series)
...
@noinline function gr_contour_levels(series::Series, clims)
...
@noinline function gr_colorbar_colors(series::Series, clims)::Vector{Int}
    if iscontour(series)
...
@noinline function _cbar_unique(values, propname)
    out = last(values)
...
@noinline gr_view_xcenter() = 0.5 * (viewport_plotarea[1] + viewport_plotarea[2])
@noinline gr_view_ycenter() = 0.5 * (viewport_plotarea[3] + viewport_plotarea[4])
...
@noinline function gr_legend_pos(v::Tuple{S,T},w,h) where {S<:Real, T<:Real}
    xpos::Float64 = v[1] * (viewport_plotarea[2] - viewport_plotarea[1]) + viewport_plotarea[1]
    ypos::Float64 = v[2] * (viewport_plotarea[4] - viewport_plotarea[3]) + viewport_plotarea[3]
    (xpos,ypos)
end
...
@noinline function gr_set_gradient(c)
    grad = _as_gradient(c)
...
@noinline function gr_set_gradient(series::Series)
    color = gr_get_color(series)
    color !== nothing && gr_set_gradient(color)
end

@noinline function gr_get_color(series::Series)
...
@noinline function gr_set_xticks_font(sp)
    flip = sp[:yaxis][:flip]
...
@noinline function gr_set_yticks_font(sp)
    flip = sp[:xaxis][:flip]
...
@noinline function gr_text_size(str)
    GR.savestate()
    GR.selntran(0)
...
@noinline function gr_text_size(str, rot)
    GR.savestate()
    GR.selntran(0)
    xs, ys = gr_inqtext(0, 0, string(str))
    l, r = extrema(xs)
...
@noinline text_box_width(w, h, rot) = abs(cosd(rot)) * w + abs(cosd(rot + 90)) * h
@noinline text_box_height(w, h, rot) = abs(sind(rot)) * w + abs(sind(rot + 90)) * h

@noinline function gr_get_ticks_size(ticks, rot)
    w, h = 0.0, 0.0
...
@noinline function gr_xaxis_height(sp)
    xaxis = sp[:xaxis]
    xticks::Tuple{Array{Float64,1},Array{String,1}}, yticks::Tuple{Array{Float64,1},Array{String,1}} = axis_drawing_info(sp)[1:2]
...
@noinline function gr_yaxis_width(sp)
    yaxis = sp[:yaxis]
    xticks::Tuple{Array{Float64,1},Array{String,1}}, yticks::Tuple{Array{Float64,1},Array{String,1}} = axis_drawing_info(sp)[1:2]
...
function _update_min_padding!(sp::Subplot{GRBackend})
    dpi::Float64 = sp.plt[:thickness_scaling]
    ...
    leftpad::AbsoluteLength   = 2mm  + sp[:left_margin]
    toppad::AbsoluteLength    = 2mm  + sp[:top_margin]
    rightpad::AbsoluteLength  = 2mm  + sp[:right_margin]
    bottompad::AbsoluteLength = 2mm  + sp[:bottom_margin]
    l::Float64 = 0.0
    h::AbsoluteLength = 0mm
    w::AbsoluteLength = 0mm
    ...
    if !(xticks in (nothing, false, :none))
        ...
        l = 0.01 + last(gr_get_ticks_size(xticks, Float64(sp[:xaxis][:rotation])))
        h = 1mm + gr_plot_size[2] * l * px
        if mirror
            toppad += h
        else
    ...
    if !(yticks in (nothing, false, :none))
        ...
        l = 0.01 + first(gr_get_ticks_size(yticks, Float64(sp[:yaxis][:rotation])))
    ...
    sp.minpad = Tuple{AbsoluteLength,AbsoluteLength,AbsoluteLength,AbsoluteLength}(dpi * [leftpad, toppad, rightpad, bottompad])
end

@noinline function viewport_calcs(viewport_plotarea,viewport_size,data_lims,ratio)
...
@noinline function legend_calc(sp,legendn,legendw,legendi)
    GR.savestate()
    GR.selntran(0)
    GR.setscale(0)
    ...
    GR.selntran(1)
    GR.restorestate()
    legendn,legendw,legendi
end

@noinline function is3d_calcs(sp,zaxis,xmin,xmax,ymin,ymax,viewport_plotarea)
    xaxis = sp[:xaxis]
    yaxis = sp[:yaxis]
    zaxis = sp[:zaxis]
...
@noinline function draw_axes_calc(sp,xmax,xmin,ymax,ymin)
    xaxis = sp[:xaxis]
    yaxis = sp[:yaxis]
    zaxis = sp[:zaxis]
...
@noinline function calc_legend(sp,legendw,legendi,legendn,clims)
    GR.savestate()
    GR.selntran(0)
    GR.setscale(0)
    gr_set_font(legendfont(sp))
...
@noinline function calc_pie(sp,series,viewport_plotarea)
    GR.selntran(0)
    GR.setfillintstyle(GR.INTSTYLE_SOLID)
...
@noinline function calc_shape(sp,series,viewport_plotarea)
...
@noinline function calc_image(sp,series,viewport_plotarea)
...
@noinline function calc_pathscatter3d(sp,series,st,viewport_plotarea,clims)
    # draw path
    if st == :path3d
        if length(x) > 1
            lz = series[:line_z]
...
@noinline function calc_heatmap(sp,series,st,viewport_plotarea,clims,xy_lims,x,y)
    zmin, zmax = clims
    if !ispolar(sp)
...
@noinline function calc_surface(st,x,y,z)
    if st == :surface
...
@noinline function calc_contour(clims,series,st,sp,x,y,z)
...
function calc_path(st,series,clims,x,y,z,frng)
...
@noinline function calc_volume(sp,y)
    sp[:legend] = :none
    GR.gr3.clear()
...
function gr_display(sp::Subplot{GRBackend}, w, h, viewport_canvas)
    ...
    if ratio != :none
        viewport_calcs(viewport_plotarea,viewport_size,data_lims,ratio == :equal ? 1.0 : ratio)
    end
    ...
    legendi::Int = 0
    if sp[:legend] != :none
        legendn,legendw,legendi = legend_calc(sp,legendn,legendw,legendi)
    end
    ...
    xmin::Float64, xmax::Float64, ymin::Float64, ymax::Float64 = data_lims
    scaleop = 0
    xtick, ytick = 1, 1
    if xmax > xmin && ymax > ymin
        (function ()
    ...
    if is3d(sp)
        is3d_calcs(sp,zaxis,xmin,xmax,ymin,ymax,viewport_plotarea)
    elseif ispolar(sp)
    ...
    elseif draw_axes
        draw_axes_calc(sp,xmax,xmin,ymax,ymin)
    end
    # end
    ...
    for (idx, series) in enumerate(series_list(sp))
        ...
        if st in (:path, :scatter, :straightline)
            calc_path(st,series,clims,x,y,z,frng)
        elseif st == :contour
            calc_contour(clims,series,st,sp,x,y,z)
        elseif st in [:surface, :wireframe]
            calc_surface(st,x,y,z)
        elseif st == :volume
            calc_volume(sp,y)
        elseif st == :heatmap
            calc_heatmap(sp,series,st,viewport_plotarea,clims,xy_lims,x,y)
        elseif st in (:path3d, :scatter3d)
            calc_pathscatter3d(sp,series,st,viewport_plotarea,clims)
        # TODO: replace with pie recipe
        elseif st == :pie
            calc_pie(sp,series,viewport_plotarea)
        elseif st == :shape
            calc_shape(sp,series,viewport_plotarea)
        elseif st == :image
            calc_image(sp,series,viewport_plotarea)
        end
    ...
    # add the legend
    if sp[:legend] != :none
        calc_legend(sp,legendw,legendi,legendn,clims)
    end
...
@noinline gr_set_output(wstype::String) = (_gr_wstype[] = wstype)
...
@noinline closeall(::GRBackend) = GR.emergencyclosegks()
