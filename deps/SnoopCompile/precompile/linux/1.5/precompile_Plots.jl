...
function _precompile_()
    ...
    Base.precompile(Tuple{Core.kwftype(typeof(Plots.plot)),NamedTuple{(:labels,),Tuple{Array{String,2}}},typeof(plot),Plots.PortfolioComposition})
    ...
    Base.precompile(Tuple{typeof(Plots._bin_centers),Array{Float64,1}})
    ...
    let fbody = try __lookup_kwbody__(which(Plots.bar, (Any,))) catch missing end
        if !ismissing(fbody)
