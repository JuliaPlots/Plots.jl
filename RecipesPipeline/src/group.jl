...
legend_entry_from_tuple(ns::NamedTuple) = join(["$k = $v" for (k, v) ∈ pairs(ns)], ", ")
...
function _extract_group_attributes(idxmap::Dict{T,V}, args...) where {T,V<:AVec{Int}}
    ...
    group_indices = Vector{Int}[collect(idxmap[k]) for k ∈ group_labels]
...
function filter_data!(plotattributes::AKW, idxfilter)
    for s ∈ (:x, :y, :z, :xerror, :yerror, :zerror)
...
function groupedvec2mat(x_ind, x, y::AbstractArray, groupby, def_val = y[1])
    ...
    fill!(y_mat, def_val)
    for i ∈ eachindex(groupby.group_labels)
...
groupedvec2mat(x_ind, x, y::Tuple, groupby) =
    Tuple(groupedvec2mat(x_ind, x, v, groupby) for v ∈ y)
...
@recipe function f(groupby::GroupBy, args...)  # COV_EXCL_LINE
    ...
    if !group_as_matrix(args[1])
        for (i, glab) ∈ enumerate(groupby.group_labels)
            @series begin
                label --> string(glab)
                idxfilter --> groupby.group_indices[i]
                for (key, val) ∈ plotattributes
        ...
        if length(g.args) == 1
            ...
            for indexes ∈ groupby.group_indices
        ...
        typeof(g)((
            x_u,
            (groupedvec2mat(x_ind, x, arg, groupby, NaN) for arg ∈ last_attrs)...,
        ))
    end
end
