...
@eval Plots ref_name(i) = "ref" * lpad(i, 3, '0')
...
function generate_cards(
    ...
    for (i, example) in enumerate(Plots._examples)
        ...
        if !isempty(example.header)
            ...
            asset = if i ∈ Plots._animation_examples
                "anim_$(backend)_$(Plots.ref_name(i)).gif"
            else
                "$(backend)_$(Plots.ref_name(i)).png"
            end
            ...
            write(
                ...
                # id: $(backend)_$(Plots.ref_name(i)) $(i ∈ skip ? "" : "\n# cover: assets/$asset")
                # author: "$(author())"
                # description: ""
                # date: $(Dates.now())
                # ---
        ...
        # """
        asset = if i ∈ Plots._animation_examples
            "gif(anim, \"assets/anim_$(backend)_$(Plots.ref_name(i)).gif\")\n"  # NOTE: must not be hidden, for appearance in the rendered `html`
        else
            "png(\"assets/$(backend)_$(Plots.ref_name(i)).png\")  #src\n"
        end
        write(
            jl, """
            mkpath("assets")  #src
        ...
        backend ≡ :plotlyjs && write(
            jl, """
            nothing  #hide
            # ![plot](assets/$(backend)_$(Plots.ref_name(i)).png)
            """
        )
    ...
    open(joinpath(cards_path, attr_name), "w") do jl
        pkg = Plots.backend_instance(Symbol(lowercase(string(backend))))
        write(
            jl, """
            # ---
    ...
    for be in bs # cols
        ...
        for (i, val) in enumerate(vals)
            be_supported_vals[i] = if func == Plots.supported_seriestypes
                stype = Plots.seriestype_supported(Plots.backend_instance(be), val)
                stype ≡ :native ? "✅" : (stype ≡ :no ? "" : "🔼")
            else
                val ∈ func(Plots.backend_instance(be)) ? "✅" : ""
            end
        end
...
function generate_supported_markdown(; default_backends)
    supported_args = OrderedCollections.OrderedDict(
        "Keyword Arguments" => (Plots._all_attrs, Plots.supported_attrs),
        "Markers" => (Plots._all_markers, Plots.supported_markers),
        "Line Styles" => (Plots._all_styles, Plots.supported_styles),
        "Scales" => (Plots._all_scales, Plots.supported_scales)
    )
